...
export default function CameraViewer({ camera, onClose, onLocate }: CameraViewerProps) {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div className="glass-panel thirdeye-glow overflow-hidden h-full flex flex-col border border-[rgba(57,255,20,0.3)]">
          {/* Header */}
          <div className="flex items-center justify-between px-3 md:px-4 py-2 md:py-3 border-b border-[var(--border-secondary)] bg-black/40">
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <div className="w-2 h-2 rounded-full bg-[#39FF14] animate-thirdeye-pulse flex-shrink-0" />
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              ...
              <button onClick={onClose} className="p-1.5 rounded hover:bg-red-900/30 transition-colors" title="Close camera viewer" aria-label="Close camera viewer">
          ...
          <div className={`relative bg-black ${fullscreen ? 'flex-1' : 'aspect-video max-h-[35vh] md:max-h-none'}`}>
            ...
            {!error && !loading && !externalOnly && (
              <div className="absolute top-2 left-2 flex items-center gap-1.5 bg-black/70 backdrop-blur-sm px-2 py-1 rounded">
                <div className="w-1.5 h-1.5 rounded-full bg-red-500 animate-thirdeye-pulse" />
