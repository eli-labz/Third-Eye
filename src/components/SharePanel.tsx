      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-12 right-0 w-72 glass-panel p-4 pointer-events-auto thirdeye-glow z-[300]"
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setIsOpen(false)} className="text-[var(--text-muted)] hover:text-[var(--text-primary)]" title="Close share panel" aria-label="Close share panel">
                <X className="w-3 h-3" />
              </button>
            </div>
            ...
            <div className="flex gap-2">
              <a
                href={`https://twitter.com/intent/tweet?text=${encodeURIComponent('🏛️ THIRD EYE — Global Intelligence Dashboard')}&url=${encodeURIComponent(generateShareUrl())}`}
                target="_blank"
              ...
              <a
                href={`https://reddit.com/submit?url=${encodeURIComponent(generateShareUrl())}&title=${encodeURIComponent('THIRD EYE — Open Source Global Intelligence Platform')}`}
                target="_blank"
