...
function ThirdEyeMap({ data, activeLayers, onEntityClick, onMouseCoords, onRightClick, onViewStateChange, flyToLocation, projection = 'globe', mapStyle = 'dark', sweepData, scanTargets = [], demoMode = false }: ThirdEyeMapProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      // ══ THIRD EYE SDK — Lattice Intelligence Mesh ══
      // ThirdEye Style: Delicate, translucent, steel-blue splined mesh
    ...
    ['sdk-sea','sdk-sea-glow','sdk-air','sdk-air-glow','sdk-intel','sdk-intel-glow'].forEach(layer => {
      map.on('click', layer, e => {
        ...
        const srcUrl = p.url || SDK_SOURCE_URLS[p.source] || 'https://geospatialcommand.center';
        ...
        popup([coords.lng, coords.lat], `<div style="${pStyle}border:1px solid ${domainColor}40;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:9px;margin-bottom:8px;">
            ...
            <div><span style="color:#5C5A54;">SOURCE</span><br/><a href="${srcUrl}" target="_blank" style="color:${domainColor};text-decoration:underline;cursor:pointer;">${p.source || 'THIRD EYE'}</a></div>
          </div>
  ...
  // ══ THIRD EYE SDK — Lattice Sensor Mesh ══
...
export default memo(ThirdEyeMap);
