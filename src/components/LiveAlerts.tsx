...
const RISK_DOT_CLASSES: Record<string, string> = {
  HIGH: 'bg-[#FF3D3D] shadow-[0_0_6px_rgba(255,61,61,0.38)]',
  CRITICAL: 'bg-[#FF1744] shadow-[0_0_6px_rgba(255,23,68,0.38)]',
  ELEVATED: 'bg-[#FF9500] shadow-[0_0_6px_rgba(255,149,0,0.38)]',
  MODERATE: 'bg-[#FFD700] shadow-[0_0_6px_rgba(255,215,0,0.38)]',
  LOW: 'bg-[#00E676] shadow-[0_0_6px_rgba(0,230,118,0.38)]',
};

const RISK_ICON_CLASSES: Record<string, string> = {
  HIGH: 'text-[#FF3D3D]',
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="gotham-tag gotham-tag--info text-[7px] px-1 py-[1px]">{BUILTIN_FEEDS.length} FEEDS</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-1.5 h-1.5 rounded-full bg-[#FF4081] animate-thirdeye-pulse" />
          <button onClick={(e) => { e.stopPropagation(); setMaximized(!maximized); if (!expanded && !maximized) setExpanded(true); }} className="hover:text-white transition-colors" title={maximized ? "Restore" : "Maximize"} aria-label={maximized ? 'Restore live alerts panel' : 'Maximize live alerts panel'}>
            {maximized ? <Minimize2 className="w-3 h-3 text-[var(--text-muted)]" /> : <Maximize2 className="w-3 h-3 text-[var(--text-muted)]" />}
          </button>
          <button
            onClick={() => setExpanded(!expanded)}
            className="hover:text-white transition-colors"
            title={expanded ? 'Collapse live alerts panel' : 'Expand live alerts panel'}
            aria-label={expanded ? 'Collapse live alerts panel' : 'Expand live alerts panel'}
          >
          ...
          >
            ...
            <div className={`flex-1 overflow-y-auto styled-scrollbar ${maximized ? 'p-6' : 'p-3'}`}>
              <div className="space-y-2">
                ...
                const sevDotClass = RISK_DOT_CLASSES[alert.severity] || RISK_DOT_CLASSES.MODERATE;
                const sevIconClass = RISK_ICON_CLASSES[alert.severity] || RISK_ICON_CLASSES.MODERATE;
                return (
                  <div
                    key={i}
                    onClick={() => {
                  ...
                  >
                    <div className="flex items-start gap-2.5">
                      ...
                      <div className="flex-shrink-0 mt-1">
                        <div className={`w-2 h-2 rounded-full ${sevDotClass}`} />
                      </div>

                      <div className="flex-1 min-w-0">
                        <div className="flex items-start gap-1.5 mb-2">
                          <Icon className={`w-3.5 h-3.5 flex-shrink-0 mt-[2px] ${sevIconClass}`} />
