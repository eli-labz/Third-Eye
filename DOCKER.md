# Self-Hosting Third Eye with Docker

Third Eye ships as a self-contained Next.js standalone build. This guide covers
running it with Docker / Docker Compose, deploying it as a [CasaOS](https://casaos.io)
app, and configuring the optional API keys.

> **TL;DR:** Third Eye runs fully **without any API keys**. All core feeds
...
- **`env_file: .env` (`required: false`)** — if a `.env` file exists its
  values are injected into the container; if it's missing, Third Eye still starts
  with the keyless feeds.
- **`ports: ${THIRDEYE_PORT:-3000}:3000`** — the web UI. The container always
  listens on 3000; the published **host** port is `THIRDEYE_PORT` (default
  `3000`). Set `THIRDEYE_PORT` in `.env` to remap it, e.g. `THIRDEYE_PORT=3005`
...
Registry, so you can run Third Eye without building anything:

```bash
docker pull ghcr.io/aiacos/third-eye:latest      # or a pinned tag, e.g. :0.1.0
docker run -d --name third-eye \
  -p 3005:3000 --env-file .env --restart unless-stopped \
  ghcr.io/aiacos/third-eye:latest
```
...
```bash
docker build -t third-eye:latest .
docker run -d --name third-eye -p 3000:3000 --env-file .env --restart unless-stopped third-eye:latest
```
...
1. On the CasaOS host, clone the repo somewhere persistent (e.g.
   `/DATA/AppData/third-eye`).
2. CasaOS dashboard → **`+`** → **Install a customized app** → paste the
   contents of `docker-compose.yml`.
   *(or simply run `docker compose up -d` from the cloned directory).*
3. Third Eye appears on the dashboard with its icon, reachable on host port
   `3000` (or whatever `THIRDEYE_PORT` you set in `.env`).
...
> directly, either build/tag `third-eye:latest` first
> (`docker build -t third-eye:latest /path/to/third-eye`) or set `image:` to a
...
rest of Third Eye works normally. Generate a key with `openssl rand -hex 32`.
...
|----------|---------|---------|
| `THIRDEYE_TELEGRAM_CHANNELS` | Comma-separated list of public Telegram channel usernames (no `@`) to scrape for the **Telegram OSINT** map layer. Overrides the curated default set. | `osintdefender,insiderpaper,aljazeeraenglish,nexta_live,war_monitor` |
| `THIRDEYE_PORT` | Host port the compose file publishes (container itself always listens on 3000). | `3000` |
